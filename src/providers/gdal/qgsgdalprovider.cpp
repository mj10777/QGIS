...
QMap<QString, QString> QgsGdalProvider::getProviderTags()
{
  return mProviderTags;
}
void QgsGdalProvider::read_providertags()
{
  if ( !mValid )
  {
    return;
  ...
  if ( GDALmetadata )
  {
    GDALDriverH driver = GDALGetDatasetDriver( mGdalBaseDataset );
    if (driver)
    {
      QString short_name = GDALGetDriverShortName( driver );
      mProviderTags.insert( "GDAL_DRIVER_SHORTNAME", short_name );
    }
    QStringList metadata = cStringList2Q_( GDALmetadata );
    if ( metadata.length() > 0 )
    {
      ...
      for ( int i = 0;i < tifftags.length();i++ )
      {
        ...
        if ( tifftag.length() > 0 )
        {
          mProviderTags.insert( tifftags[i].section( '=', 0, 0 ), tifftags[i].section( '=', 1 ) );
        }
      }
