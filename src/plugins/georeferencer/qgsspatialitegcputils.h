...
#ifndef QGSSPATIALITEGCPUTILS_H
#define QGSSPATIALITEGCPUTILS_H
...
/**
  ...
  *  - with a Spatialite version compiled with the Gcp-logic, the georeferencer can calculate the corresponding pixel-point
  * \since QGIS 3.0
  */
class QgsSpatiaLiteGcpUtils
{
  public:
     ...
     * \since QGIS 3.0
     */
    static bool ConvertLength( double inputValue, QgsSpatiaLiteGcpUtils::ConvertUnits unitFrom, QgsSpatiaLiteGcpUtils::ConvertUnits unitTo, double *outputValue );
     ...
     *  -> the use of the connection is only intended for internal use
     * \since QGIS 3.0
     */
    enum GcpDatabases
    {
    ...
    struct GcpDbData
    {
      GcpDbData( QString sDatabaseFilename, QString sCoverageName = QString::null, int i_srid = INT_MIN )
        ...
        , mLayer( nullptr )
        , mGcpEnabled( false )
        , mInputPoint( QgsPointXY( 0.0, 0.0 ) )
        , mToPixel( false )
        , mOrder( 0 )
        , mReCompute( true )
        , mIdGcp( -1 )
      ...
      QString mGcpCoverageName; // file without extension Lower-Case
      int mGcpSrid;
      int mGcpMasterSrid;
     ...
     *  -> and should be 'transferred' to the member variablen of the Application
     * \note
     *  this function will be called
     *  -> during an open, change or close
     * \see createGcpDb
     ...
     * \since QGIS 3.0
     */
    static QgsPointXY getGcpConvert( GcpDbData *parmsGcpDbData );
     ...
     *  The intention is to create a reusable Database of Well-Known-Points
     ...
     *  --> many SpatialView's are created to SELECT only specific gcp_points based on the 'gcp_type'
     *  ---> see the CREATE Statement of 'gcp_master' for the supported values
     * \note
     ...
     * \note
     *  - When the Spatialite Gcp-Logic is being used and returns useful results
  ...
  private:
    ...
    /**
     * Creates sql to CREATE a 'gcp_points_||gcp_coverage' TABLE, TRIGGERS and VIEWs for the corresponding gcp_coverage entry
     *  - goal is the maintenance of the field-name syntax is in one place
     ...
     *  -> when Spatialite Gcp-logic is active: also CREATE Statement for 'gcp_compute' TABLE
     ...
     *  -> 2 results returned: 'id_gcp_coverage' and all values (including 'id_gcp_coverage') separated with 'parmsGcpDbData->mParseString'
     ...
     * \param sSqlOutput collection of Sql-Statements to be dumped to file
     * \param sFileExtension the extension to use to replace the source file extension with
     * \return file_result result of QFile error() by open/output commands
     * \since QGIS 3.0
     */
    static QFile::FileError writeGcpSqlDump( QString sSourceFilename, QString sSqlOutput = QString::null, QString sFileExtension = "sql" );
     ...
     *  - when the static function(s) are completed, the connection should be closed
     ...
     *  TIFFTAG_DOCUMENTNAME, IMAGEDESCRIPTION and COPYRIGHT
     *  - can be used to set Title, Abstract and KeywordsList
     * \note will only be called when provider == "gdal"
     * \note a 'copyright' should be added to store the value of TIFFTAG_COPYRIGHT
     *  - which is often a requirement of OpenData sources
     * @see setDataProvider
     * @param rasterLayerbeing created
     * @param b_SetTags when true: will set Title, Abstract and KeywordsList from TIFFTAG_DOCUMENTNAME, IMAGEDESCRIPTION and COPYRIGHT if found
