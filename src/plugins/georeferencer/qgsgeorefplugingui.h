...
class QgsGeorefPluginGui : public QMainWindow, private Ui::QgsGeorefPluginGuiBase
{
  ...
  private slots:
    ...
    * @see mMapCanvasLayers
     */
    void visibilityLayerTreeViewChanged( QgsLayerTreeNode *layerTreenode );
    // gcps
     ...
     */
    void addPoint( const QgsPointXY &pixelCoords, const QgsPointXY &mapCoords, int id_gcp = -1, bool b_PixelMap = true,
     ...
     *  -- after 'commitChanges' has completed, the value in 'mEventGcpStatus' will be used to retrieve the inserted primary-key
     ...
     * - which will also save any new position that were not committed after 'geometryChanged_gcp'
     * \see getGcpConvert
     ...
     * \param changed_geometry the changed geometry value must be updated in the gcp-list
     */
    void geometryChanged_cutline( QgsFeatureId fid, const QgsGeometry &changed_geometry );
     ...
     * \since QGIS 3.0
     */
    void zoomGcpPixelsExtent();

    /**
     * showGcpPixelsPropertiesDialog
     * \since QGIS 3.0
     */
    void showGcpPixelsPropertiesDialog();
     ...
     *  - prepares auxiliary file names (.point etc)
     *  - calls addRaster
    ...
    /**
     * jumpToGCP
     * move to given point in Georeferencer Canvas
     * \note
     *  - if LinkGeorefToQGis is selected, move to given point in Qgis Canvas
     * \since QGIS 3.0
     * \see QgsGeorefDataPoint::moveTo
     */
  ...
  private:
     ...
     * \see extentsChanged
     * \see loadGCPs
     * \param rasterFileName file-name as string
     */
    void addRaster( const QString &rasterFileName );
     ...
     *  - a (possibly changed) value is NOT stored in the QqsSettings
     * \since QGIS 3.0
     * New for ( mLegacyMode == 1 )
     *  - therefore is needed
     ...
     *  - therefore is needed
     */
    void jumpToGcpConvert( QgsPointXY inputPoint, bool bToPixel = false );
    ...
    /**
     * generateGcpList
     *  - for gdal_translate or as separate list
     * \note
     ...
     *  - The last found id_cutline that fulfills the following conditions
     ...
     * \see generateGDALScript()
     * \see QgsSpatiaLiteGcpUtils::updateGcpDb
     ...
     * \note
     * The User may change this value before calling 'fetchGcpMasterPointsExtent'
     *  - therefore the, possibly changed value, must be retrieved before first usage
     * \see mGcpMasterArea
     * \see fetchGcpMasterPointsExtent
     */
    ...
    /**
     * Simplify the commit of the gcp-layers, with checking and committing as needed
     * \note
     ...
     * --> uncommitted data in the 'VectorLayerEditBuffer' cannot be evaluated
     * this is the main reason for committing the data after each change
     * - which is contrary to the normal behavior in the QGIS-Application
     * \note
     ...
     * \note
     * - this function cause Appllication crashes in its initial form
    ...
    * @see setCutlineLayerSettings
     ...
     *  The column 'coverage_name' is based on the lowercase (original) file-name WITHOUT path and and extension
     ...
     * \param sCoverageName Gcp-Points TABLE to update
     * \since QGIS 3.0
     * \see QgsSpatiaLiteGcpUtils::updateGcpCompute
     */
     ...
     * \param sCoverageName Gcp-Points TABLE to update
     * \since QGIS 3.0
     * \see QgsSpatiaLiteGcpUtils::updateGcpTranslate
     */
     ...
     * \return QgsPointXY of result (0,0 when invalid)
     * \since QGIS 3.0
     * \see QgsSpatiaLiteGcpUtils::getGcpConvert
     */
    QgsPointXY getGcpConvert( QString sCoverageName, QgsPointXY inputPoint, bool bToPixel = false, int i_order = 0, bool bReCompute = true, int id_gcp = -1 );
     ...
     */
    double getMetersToMapPoint( double d_GcpMasterArea = 5.0, int i_srid = -1,  QgsPointXY inputPoint = QgsPointXY( 0.0, 0.0 ) );
